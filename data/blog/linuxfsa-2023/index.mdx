...
-   _sudo or wheel_ : Members of the sudo (or wheel) group have the authority to execute commands with elevated privileges using sudo.
-   _adm_ : The adm group typically has read access to system log files.
-   _shadow_ : The shadow group is related to managing user authentication and password information. With this membership, a user can read the /etc/shadow file, which contains the password hashes of all users on the system.
-   _disk_ : Members of the disk group have almost unrestricted read and limited write access inside the system.
